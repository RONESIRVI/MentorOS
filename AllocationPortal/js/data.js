...
const RoneData = (() => {
  ...
  function addStudent(raw) {
    const student = {
      id:        genStudentId(),
      name:      (raw.name || '').trim(),
      class:     (raw.target_year || raw.class || raw['class'] || '2025').trim(),
      stream:    (raw.medium || raw.stream || 'Hindi').trim(),
      subject:   (raw.optional_subject || raw.subject || '').trim(),
      goal:      (raw.exam_goal || raw.goal || 'UPSC CSE').trim(),
      city:      (raw.city || '').trim(),
  ...
  function addMentor(raw) {
    const mentor = {
      id:             genMentorId(),
      name:           (raw.name || '').trim(),
      specialisation: (raw.expertise || raw.specialisation || raw.spec || 'General Guidance').trim(),
      institution:    (raw.experience || raw.institution || '').trim(),
